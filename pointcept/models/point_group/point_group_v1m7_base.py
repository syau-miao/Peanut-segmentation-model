...
@MODELS.register_module("PG-v1m7")
class PointGroup(nn.Module):
    ...
    def loss(self, logit_pred, bias_pred, data_dict):
        ...
        if self.training:
            ...
            """
            t = (coord[:, 2] - instance_bottom[:, 2]) / instance_stem_v[:, 2]
            bias_gt_x = instance_bottom[:, 0] + t * instance_stem_v[:, 0] - coord[:, 0]
            bias_gt_y = instance_bottom[:, 1] + t * instance_stem_v[:, 1] - coord[:, 1]
            bias_gt_z = torch.zeros_like(bias_gt_x)
            bias_gt = torch.stack([bias_gt_x, bias_gt_y, bias_gt_z], dim=1)
            """
            t = (coord[:, 2] - instance_bottom[:, 2]) / instance_stem_v[:, 2]
            # 直接计算bias_gt，利用广播机制处理x，y维度，z维度设为0
            bias_gt = (instance_bottom + t.unsqueeze(1) * instance_stem_v - coord)
            bias_gt[:, 2] = 0  # 确保z维度的偏移为0

            # coord_B_v = coord - instance_bottom
            # bias_gt = (coord_B_v * instance_stem_v) * instance_stem_v - coord_B_v
        else:
            instance_centroid = data_dict["instance_centroid"]
