...
@MODELS.register_module("PG-v4m1")
class PointGroup(nn.Module):
    def __init__(
        ...
        # class_weights = torch.from_numpy(np.array(class_weights, dtype=np.float32))
        # self.ce_criteria = torch.nn.CrossEntropyLoss(ignore_index=semantic_ignore_index, weight=class_weights)
    ...
    def pred_feat(self, bias_pred, data_dict):
        coord = data_dict["coord"]
        offset = data_dict["offset"]
        bias_pred[:, :2] = 0
        center_pred = coord + bias_pred
        center_pred /= self.voxel_size
        logit_pred = torch.zeros(coord.size(0)).reshape(-1, 1)
        segment_pred = torch.zeros(coord.size(0), dtype=torch.int32)  # [n]
        # cluster
        mask = (
            ~torch.concat(
                [
    ...
    def forward(self, data_dict):
        ...
        if not self.training:
            pred_dict = self.pred_feat(bias_pred, data_dict)
