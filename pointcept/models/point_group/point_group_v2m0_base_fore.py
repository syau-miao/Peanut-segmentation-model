...
def find_clusters_min_z(coord, crop_z, eps=0.1, min_samples=10):
    ...
    # visualize_clusters_xy(coord, cluster_labels, cluster_min_idx)

    return cluster_min_idx


def visualize_clusters_xy(coord, cluster_labels, cluster_min_idx):
    # 设置颜色映射
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(coord[:, 0], coord[:, 1], c=cluster_labels, cmap='viridis', alpha=0.6, edgecolor='k')

    # 绘制每个聚类的z最小点
    min_points = coord[cluster_min_idx]
...
@MODELS.register_module("PG-v2m0-fore")
class PointGroup(nn.Module):
    ...
    def pred_feat_m(self, bias_pred, data_dict):
        ...
        z_min = center_pred_np[:, 2].max() / 3

        min_z_per_cluster = find_clusters_min_z(center_pred_np, crop_z=10, eps=radius, min_samples=self.cluster_min_points)
