...
class SemSegInstPostTester(TesterBase):
    ...
    def sor_filter(self, mask_coord, k=8, z_threshold=1.0):
        """
        应用 SOR 滤波来去除 mask_coord 中的离群点
        :param mask_coord: 当前掩码对应的点云坐标
        :param k: 邻域内的点的数量，默认 8
        :param z_threshold: 距离阈值，表示标准差的倍数，默认 1.0
        :return: 去噪后的 mask_coord
        """
        if mask_coord.shape[0] == 0:
            return mask_coord  # 如果没有点，直接返回原数据

        # 使用 KNN 找到每个点的 k 个最近邻
        nbrs = NearestNeighbors(n_neighbors=k).fit(mask_coord)
        distances, _ = nbrs.kneighbors(mask_coord)

        # 计算每个点的平均距离
        mean_distances = np.mean(distances[:, 1:], axis=1)  # 排除自身的距离

        # 计算距离的标准差
        distance_mean = np.mean(mean_distances)
        distance_std = np.std(mean_distances)

        # 根据 z_threshold 识别离群点（噪声）
        inlier_mask = mean_distances < (distance_mean + z_threshold * distance_std)

        # 返回去除噪声后的点
        return mask_coord[inlier_mask]

    def post_seg(self, input_dict, pred_dict, fra_i=0, save_inst=None):
        ...
        for instance_id, mask in enumerate(pred_masks):
            ...
            if mask_coord[:, 2].max() < 0.45:
                semantic_class = -1
                instance_id = -3
            pred[mask == 1] = semantic_class
            pred_inst[mask == 1] = instance_id + 1  # 实例ID从0开始

        other_mask = pred == -1
        if np.any(other_mask):  # 检查是否有未分类点
            pred_logits[other_mask, 0] = 1  # 设置地面类别的 logits 为高值
            pred_logits[other_mask, 1] = pred_logits.min()  # 非地面类别的 logits 设置为低值
            # pred[other_mask] = 0  # 这些点的语义标签设为地面
        ...
        # pred = np.ones((coord.shape[0], 2)) * -1
        #
        # ground_mask = pred_sem_class==0
        # pred[ground_mask] = 0
        # for instance_id, mask in enumerate(pred_masks):
        #     semantic_class = pred_classes[instance_id]
        #     pred[mask == 1, 0] = semantic_class
        #     pred[mask == 1, 1] = instance_id+1  # 实例ID从0开始

        # pred_center = coord + pred_dict["bias_pred"].cpu().numpy()
        #
        # # post plant
        # # 保存每个分割的结果
        ...
        # ground_mask = pred_sem_class==0
        # pred_data[ground_mask, 6] = 0
        # pred_data[ground_mask, 7] = 0
        # for instance_id, mask in enumerate(pred_masks):
        #     semantic_class = pred_classes[instance_id]
        #     pred_data[mask == 1, 6] = semantic_class
        #     pred_data[mask == 1, 7] = instance_id  # 实例ID从0开始
        #
        # # show_pcd(pred_center, pred_data[:, 7])
        # np.savetxt(f"./tmp/inst_{data_name}.txt", pred_data, fmt="%.6f")
        # return pred[:, 0]
